...
ready(() => {
	...
	if (ifSelectorExist(containerPostFields)) {
		Array.from(containerPostFields).forEach(input => {
			input.value = clixsy_js_data.page_id;
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  telInputs.forEach(function(input) {
      ...
      // Existing input event listener to restrict input
      ...
      // New blur event listener to add red border for validation
      input.addEventListener('blur', function(e) {
          if (e.target.value.length !== 10) {
              e.target.style.border = '1px solid red';
          } else {
              e.target.style.border = ''; // Reset to default or whatever you want
          }
      });
  });
});
